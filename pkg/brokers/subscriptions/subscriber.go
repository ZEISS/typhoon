...
func materializeSubscriptionsAPIFilter(ctx context.Context, filter cfgbroker.Filter) eventfilter.Filter {
	...
	case len(filter.Exact) > 0:
		...
		materializedFilter, err = subscriptionsapi.NewExactFilter("type", filter.Exact["type"])
		if err != nil {
	...
	case len(filter.Prefix) > 0:
		// The webhook validates that this map has only a single key:value pair.
		materializedFilter, err = subscriptionsapi.NewPrefixFilter("type", filter.Prefix["type"])
		if err != nil {
	...
	case len(filter.Suffix) > 0:
		// The webhook validates that this map has only a single key:value pair.
		materializedFilter, err = subscriptionsapi.NewSuffixFilter("type", filter.Suffix["type"])
		if err != nil {
