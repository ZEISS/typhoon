...
import (
	...
	"github.com/zeiss/typhoon/pkg/reconciler/common"
...
func buildRedisDeployment(rb *eventingv1alpha1.RedisBroker, image string) *appsv1.Deployment {
	return resource.NewDeployment(rb.Namespace, rb.Name+"-"+redisResourceSuffix,
		resource.DeploymentWithMetaOptions(
			resource.MetaAddLabel(resource.AppNameLabel, common.AppAnnotationValue(rb)),
			resource.MetaAddLabel(resource.AppComponentLabel, "redis-deployment"),
			...
			resource.MetaAddLabel(resource.AppInstanceLabel, rb.Name+"-"+redisResourceSuffix),
			resource.MetaAddOwner(rb, rb.GetGroupVersionKind())),
		resource.DeploymentAddSelectorForTemplate(resource.AppComponentLabel, "redis-deployment"),
		resource.DeploymentAddSelectorForTemplate(resource.AppInstanceLabel, rb.Name+"-"+redisResourceSuffix),
		...
		resource.DeploymentWithTemplateSpecOptions(
			...
			resource.PodTemplateSpecWithPodSpecOptions(
				resource.PodSpecAddContainer(
					resource.NewContainer("redis", image,
						resource.ContainerAddEnvFromValue("REDIS_ARGS", "--appendonly yes"),
						resource.ContainerAddPort("redis", 6379))))))
}
...
func buildRedisService(rb *eventingv1alpha1.RedisBroker) *corev1.Service {
	return resource.NewService(rb.Namespace, rb.Name+"-"+redisResourceSuffix,
		resource.ServiceWithMetaOptions(
			resource.MetaAddLabel(resource.AppNameLabel, common.AppAnnotationValue(rb)),
			resource.MetaAddLabel(resource.AppComponentLabel, "redis-service"),
			resource.MetaAddLabel(resource.AppPartOfLabel, resource.PartOf),
			resource.MetaAddLabel(resource.AppManagedByLabel, resource.ManagedBy),
			resource.MetaAddLabel(resource.AppInstanceLabel, rb.Name+"-"+redisResourceSuffix),
			resource.MetaAddOwner(rb, rb.GetGroupVersionKind())),
		resource.ServiceSetType(corev1.ServiceTypeClusterIP),
		resource.ServiceAddSelectorLabel(resource.AppComponentLabel, "redis-deployment"),
		resource.ServiceAddSelectorLabel(resource.AppInstanceLabel, rb.Name+"-"+redisResourceSuffix),
		resource.ServiceAddPort("redis", 6379, 6379))
}
