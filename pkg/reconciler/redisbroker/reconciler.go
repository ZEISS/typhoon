...
func redisDeploymentOption(rb *eventingv1alpha1.RedisBroker, redisSvc *corev1.Service) resource.ObjectOption {
	return func(obj interface{}) {
		...
		if rb.IsUserProvidedRedis() {
			...
			if rb.Spec.Redis.Connection.ClusterURLs != nil &&
				...
				resource.ContainerAddEnvFromValue("REDIS_CLUSTER_ADDRESSES",
					strings.Join(rb.Spec.Redis.Connection.ClusterURLs, ","))(c)
			} else {
				resource.ContainerAddEnvFromValue("REDIS_ADDRESS", *rb.Spec.Redis.Connection.URL)(c)
			}

			if rb.Spec.Redis.Connection.Username != nil {
				resource.ContainerAddEnvVarFromSecret("REDIS_USERNAME",
			...
			if rb.Spec.Redis.Connection.Password != nil {
				resource.ContainerAddEnvVarFromSecret("REDIS_PASSWORD",
			...
			if rb.Spec.Redis.Connection.TLSCACertificate != nil {
				resource.ContainerAddEnvVarFromSecret("REDIS_TLS_CA_CERTIFICATE",
			...
			if rb.Spec.Redis.Connection.TLSCertificate != nil {
				resource.ContainerAddEnvVarFromSecret("REDIS_TLS_CERTIFICATE",
			...
			if rb.Spec.Redis.Connection.TLSKey != nil {
				resource.ContainerAddEnvVarFromSecret("REDIS_TLS_KEY",
			...
			if rb.Spec.Redis.Connection.TLSEnabled != nil && *rb.Spec.Redis.Connection.TLSEnabled {
				resource.ContainerAddEnvFromValue("REDIS_TLS_ENABLED", "true")(c)
			}
			...
			if rb.Spec.Redis.Connection.TLSSkipVerify != nil && *rb.Spec.Redis.Connection.TLSSkipVerify {
				...
				resource.ContainerAddEnvFromValue("REDIS_TLS_SKIP_VERIFY", tlsSkipVerifyDefault)(c)
			}

		} else {
			resource.ContainerAddEnvFromValue("REDIS_ADDRESS",
