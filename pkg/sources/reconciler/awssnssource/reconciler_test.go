...
type (
	mockTopics             map[ /*arn*/ string]*sns.GetTopicAttributesOutput
	mockSubscriptionsPages map[ /*token*/ *string][]*sns.Subscription
)
...
func makeMockTopics(topicExists bool) map[string]interface{} {
	...
	wrongTopicARN := "aws:sns:not:my:topic"
	okTopicARN := tTopicARN.String()
...
func makeMockSubscriptionsPages(subExists bool) map[string]interface{} {
	...
	wrongSubURL := aws.String("http://not-my-sub.example.com")
	wrongSubARN := aws.String("aws:sns:not:my:sub")

	okSubURL := aws.String(tAdapterURI.String() + tNs + "/" + tName)
	okSubARN := aws.String(tSubARN.String())
