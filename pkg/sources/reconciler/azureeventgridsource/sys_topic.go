...
func EnsureSystemTopic(ctx context.Context, cli eventgrid.SystemTopicsClient,
	...
	resGroupsCli eventgrid.ResourceGroupsClient,
) (*v1alpha1.AzureResourceID /*sysTopicResID*/, error) {
	if skip.Skip(ctx) {
		return nil, nil
	}
...
func EnsureNoSystemTopic(ctx context.Context, cli eventgrid.SystemTopicsClient,
	eventSubsCli eventgrid.EventSubscriptionsClient, sysTopic *azureeventgrid.SystemTopic,
) reconciler.Event {
	if skip.Skip(ctx) {
		return nil
	}
...
) (*azureeventgrid.SystemTopic, error) {
	restCtx, cancel := context.WithTimeout(ctx, crudTimeout)
	defer cancel()
...
func getResourceRegion(ctx context.Context, cli autorest.Client,
	scope v1alpha1.AzureResourceID, apiVersion string,
) (string, error) {
	restCtx, cancel := context.WithTimeout(ctx, crudTimeout)
	defer cancel()
